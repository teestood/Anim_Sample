...
// Copy of default NetworkDebugStartGUI class shipped with Fusion. 
// Changes: Removed shared mode possibility as it is not supported in Projectiles,
// added option to quickstart Host with 1 or 2 clients
namespace Animations
{
  ...
  public class NetworkDebugStartGUI : Behaviour
  {
      ...
      GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
      {
        ...
        GUILayout.BeginVertical(GUI.skin.window);
        {
          ...
          if (currentstage == NetworkDebugStart.Stage.Disconnected)
          {
            ...
            if (_isMultiplePeerMode == true && GUILayout.Button("Start Host + 1 Client", GUILayout.Height(height)))
            {
              nds.StartHostPlusClients(1);
            }

            if (_isMultiplePeerMode == true && GUILayout.Button("Start Host + 2 Clients", GUILayout.Height(height)))
            {
              nds.StartHostPlusClients(2);
            }
