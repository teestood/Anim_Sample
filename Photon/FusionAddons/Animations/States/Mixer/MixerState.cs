namespace Fusion.Animations
{
	...
	public class MixerState : AnimationState, IAnimationStateOwner
	{
		...
		void IAnimationStateOwner.Activate(AnimationState source, float fadeDuration)
		{
			...
			Controller.Log($"{nameof(MixerState)}.{nameof(Activate)} ({name}), Fade Duration: {fadeDuration:F3}", gameObject);
		...
		void IAnimationStateOwner.Deactivate(AnimationState source, float fadeDuration)
		{
			...
			Controller.Log($"{nameof(MixerState)}.{nameof(Deactivate)} ({name}), Fade Duration: {fadeDuration:F3}", gameObject);
