...
namespace Fusion.FSM
{
	public abstract class StateBehaviour<TState> : NetworkBehaviour, IState, IOwnedState<TState> where TState : StateBehaviour<TState>
	{
		...
		protected virtual bool CanExitState(TState nextState, EForceBehaviour force) => true;
		...
		bool IState.CanExitState(IState nextState, EForceBehaviour force)
		{
			if (force == EForceBehaviour.None && _checkPriorityOnExit == true && (nextState as TState).Priority < _priority)
				return false;

			return CanExitState(nextState as TState, force);
		}
		...
		private bool TryTransition(ref TransitionData<TState> transition)
		{
			...
			if (CanExitState(transition.TargetState, EForceBehaviour.Transition) == false)
				return false;
