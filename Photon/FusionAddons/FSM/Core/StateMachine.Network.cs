...
namespace Fusion.FSM
{
	public partial class StateMachine<TState>
	{
		...
		unsafe void IStateMachine.Read(int* ptr)
		{
			...
			if (_hasChildMachines == true)
			{
				for (int i = 0; i < _stateCount; i++)
				{
					...
					{
						state.ChildMachines[j].Read(ptr);
					}
				}
			}
		}
			...
			*ptr = _bitState;
			...
			if (_hasChildMachines == true)
			{
				for (int i = 0; i < _stateCount; i++)
				{
					...
					for (int j = 0; j < state.ChildMachines.Length; j++)
					{
						state.ChildMachines[j].Write(ptr);
					}
				}
			}
		}
		...
		unsafe void IStateMachine.Interpolate(InterpolationData interpolationData)
		{
			...
			_interpolationTick = UnityEngine.Mathf.Lerp(interpolationData.FromTick, interpolationData.ToTick, interpolationData.Alpha);

			// We prefer using only From values
			bool useFrom = true;

			_activeStateId = useFrom == true ? *interpolationData.From : *interpolationData.To;
			interpolationData.From++;
			interpolationData.To++;

			_previousStateId = useFrom == true ? *interpolationData.From : *interpolationData.To;
			interpolationData.From++;
			interpolationData.To++;

			_stateChangeTick = useFrom == true ? *interpolationData.From : *interpolationData.To;
			interpolationData.From++;
			interpolationData.To++;

			_bitState = useFrom == true ? *interpolationData.From : *interpolationData.To;
			interpolationData.From++;
			interpolationData.To++;
			...
			if (_hasChildMachines == true)
			{
				for (int i = 0; i < _stateCount; i++)
				{
					...
					for (int j = 0; j < state.ChildMachines.Length; j++)
					{
						state.ChildMachines[j].Interpolate(interpolationData);
					}
				}
			}
		}
