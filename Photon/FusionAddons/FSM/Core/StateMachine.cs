...
namespace Fusion.FSM
{
	public partial class StateMachine<TState> : IStateMachine where TState : class, IState
	{
		...
		private readonly int _defaultState = 0;
		...
		{
			return TryActivateState(stateId, allowReset, EForceBehaviour.None);
		}
		...
		public bool TryDeactivateState(int stateId)
		{
			...
			return TryActivateState(_defaultState, false, EForceBehaviour.None);
		}
		...
		public bool ForceDeactivateState(int stateId)
		{
			...
			return ForceActivateState(_defaultState);
		}
		...
		public bool TryToggleState(int stateId, bool value)
		{
			...
			int targetState = value == true ? stateId : _defaultState;
			return TryActivateState(targetState, false, value == false ? EForceBehaviour.ToggleOff : EForceBehaviour.None);
		}
		...
		public void Reset()
		{
			_activeStateId = _defaultState;
			_previousStateId = _defaultState;
			_stateChangeTick = 0;
			_bitState = 0;
		...
		private bool TryActivateState(int stateId, bool allowReset, EForceBehaviour force)
		{
			if (stateId == ActiveStateId && allowReset == false)
				return false;
			...
			if (ActiveState.CanExitState(nextState, force) == false)
				return false;
		...
		private void ChangeState(int stateId)
		{
			...
			Assert.Check(Runner.Simulation.Stage != default, "State changes are not allowed from Render calls");
			...
			_stateChangeTick = Runner.Simulation.Tick;
		...
		{
			int currentTick = Runner.Stage == default && _interpolationTick != 0f ? (int)_interpolationTick : Runner.Simulation.Tick;
		...
		private float GetStateTime()
		{
			if (Runner.Stage != default || _interpolationTick == 0f)
				return (Runner.Simulation.Tick - StateChangeTick) * Runner.Simulation.DeltaTime;

			return (_interpolationTick - StateChangeTick) * Runner.Simulation.DeltaTime;
		}
		...
		private void LogStateChange()
		{
			...
			if (HasStarted == true)
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#F04C4C>State Machine {Name}: Change State to {ActiveState.Name}</color> - Previous: {PreviousState.Name}, Tick: {Runner.Simulation.Tick.Raw}", _controller);
			}
			else
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#F04C4C>State Machine {Name}: Change State to {ActiveState.Name}</color> - Tick: {Runner.Simulation.Tick.Raw}", _controller);
			}
		}
		...
		private void LogRenderStateChange()
		{
			...
			if (_lastRenderStateId >= 0)
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#467DE7>State Machine {Name}: Change RENDER State to {ActiveState.Name}</color> - Previous: {_states[_lastRenderStateId].Name}, StateChangeTick: {_stateChangeTick}, RenderFrame: {Time.frameCount}", _controller);
			}
			else
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#467DE7>State Machine {Name}: Change RENDER State to {ActiveState.Name}</color> - StateChangeTick: {_stateChangeTick}, RenderFrame: {Time.frameCount}", _controller);
			}
		}
	}
}
