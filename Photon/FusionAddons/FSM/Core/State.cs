...
namespace Fusion.FSM
{
	public abstract unsafe class State<TState> : IState, IOwnedState<TState> where TState : State<TState>
	{
		...
		protected virtual bool CanExitState(IState nextState, EForceBehaviour force) => true;
		...
		{
			if (force == EForceBehaviour.None && CheckPriorityOnExit == true && (nextState as TState).Priority < Priority)
				return false;

			return CanExitState(nextState, force);
		}
